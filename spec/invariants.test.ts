...
const pages = shipped
  ...
  .map((name) => ({
    name,
    // A deck is a standalone presentation surface, not a page of the site: it
    // loads the brand CSS and src/decks/theme.css only --- not the site's
    // stylesheets, not its head partial, not its nav --- and it carries one
    // <h1> per slide. Three invariants below therefore don't apply to it. The
    // rest do, and still run. Delete this flag if decks ever get the site
    // chrome.
    isDeck: name.startsWith("decks/"),
